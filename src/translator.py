...
def translate(content: str) -> tuple[bool, str]:
    """Determines if content is in English and translates if not."""
...
def get_language(post: str) -> str:
    """Detects if text is in English or Non-English using LLM."""
    prompt = f"Classify the following text as 'English' or 'Non-English':\n{post}"
    ...
    try:
        ...
        if response.choices:
            classification = response.choices[0].message.content.strip().capitalize()
